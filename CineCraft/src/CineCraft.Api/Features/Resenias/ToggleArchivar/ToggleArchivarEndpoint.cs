using System.IdentityModel.Tokens.Jwt;
using CineCraft.Api.Data;
using Microsoft.EntityFrameworkCore;

namespace CineCraft.Api.Features.Resenias.ToggleArchivar;

public static class ToggleArchivarEndpoint
{
    public static void MapToggleArchivar(this IEndpointRouteBuilder app)
    {
        app.MapPatch("/{id:int}/archivar", async (
            int id,
            HttpContext httpContext,
            CineCraftContext dbContext) =>
        {
            var userIdClaim = httpContext.User
                .FindFirst(JwtRegisteredClaimNames.Sub)?.Value;

            if (!int.TryParse(userIdClaim, out var userId))
            {
                return Results.Unauthorized();
            }

            var resenia = await dbContext.Resenias
                .FirstOrDefaultAsync(r =>
                    r.Id == id &&
                    r.UsuarioAutorId == userId &&
                    !r.Deleted);

            if (resenia is null)
            {
                return Results.NotFound(new
                {
                    message = "La reseña no existe o no pertenece al usuario."
                });
            }

            resenia.Archivada = !resenia.Archivada;
            await dbContext.SaveChangesAsync();

            var estado = resenia.Archivada ? "archivada" : "desarchivada";
            return Results.Ok(new ToggleArchivarResponse(
                resenia.Id,
                resenia.Archivada,
                $"La reseña ha sido {estado} exitosamente."
            ));
        });
    }
}